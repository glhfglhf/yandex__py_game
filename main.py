...
class AboutProject(QMainWindow):
    def __init__(self):
        super().__init__()
        uic.loadUi('about_project.ui', self)
...
class Sapper:
    def __init__(self, width, height, mines):
        global count_of_flags, count_of_lives, count_of_opened_cells
    ...
    # подсчет поличества соседей
    def number_of_neighbors(self, cell):
        number = 0
        try:
            number += (self.board[cell[1] - 1][cell[0] - 1] == 10)
        except IndexError:
            pass
        try:
            number += (self.board[cell[1]][cell[0] - 1] == 10)
        except IndexError:
            pass
        try:
            number += (self.board[cell[1] + 1][cell[0] - 1] == 10)
            ...
            pass
        try:
            number += (self.board[cell[1] - 1][cell[0]] == 10)
        except IndexError:
            pass
        try:
            number += (self.board[cell[1] + 1][cell[0]] == 10)
...
path = resource_path(os.path.join('Folder', 'icon.jpg'))
